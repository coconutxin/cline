...
import {
	...
	type WorkspaceAffinityCheckResult,
} from "./task/workspaceAffinity";
...
export class Controller {
	...
	*/
	...
	async reinitExistingTaskFromId(taskId: string) {
		...
		if (history) {
			if (
				!(await this.ensureHistoryItemMatchesCurrentWorkspace(
					history.historyItem,
				))
			) {
				return;
	...
	async ensureHistoryItemMatchesCurrentWorkspace(
		historyItem: HistoryItem,
	): Promise<boolean> {
	...
	private async handleHistoryItemWorkspaceMismatch(
		...
		const currentWorkspaces =
			affinity.currentWorkspacePaths.join(", ") || "no open workspace";
		Logger.warn(
	...
	async cancelTask() {
		...
		try {
			...
			if (
				historyItem &&
				(await this.ensureHistoryItemMatchesCurrentWorkspace(historyItem))
			) {
