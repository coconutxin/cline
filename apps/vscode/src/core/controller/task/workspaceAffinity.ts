...
export function getHistoryItemWorkspaceAffinityPath(historyItem: HistoryItem): string | undefined {
	return nonEmptyPath(historyItem.cwdOnTaskInitialization) ?? nonEmptyPath(historyItem.shadowGitConfigWorkTree)
}
...
export function checkHistoryItemWorkspaceAffinity(
	...
	const currentWorkspacePaths = workspaceRoots.map((root) => nonEmptyPath(root.path)).filter((path): path is string => !!path)
	...
	const matchedRootPath = currentWorkspacePaths.find((workspacePath) => arePathsEqual(taskWorkspacePath, workspacePath))
