...
} from "@shared/storage/provider-keys";
...
export class Task {
	...
	private createApiRetryAttemptHandler(): NonNullable<
		ApiConfiguration["onRetryAttempt"]
	> {
		return async (
			attempt: number,
			maxRetries: number,
	...
	private getCurrentProviderInfo(): ApiProviderInfo {
		...
		const activeProviderId =
			this.taskState.toolUseFailureFallbackActive &&
	...
	private buildToolUseFailureFallbackApiHandler():
		...
		if (
			mode !== "act" ||
			this.taskState.toolUseFailureFallbackUsed ||
			this.taskState.toolUseFailureFallbackActive
		) {
			return undefined;
		}
		...
		const configuredFallbackModelId =
			apiConfig.toolUseFailureFallbackApiModelId?.trim();
		const fallbackModelId =
			configuredFallbackModelId ||
			getProviderDefaultModelId(fallbackProvider) ||
			"";
		if (!fallbackModelId) {
			return undefined;
		}
		...
		if (
			fallbackProvider === primaryInfo.providerId &&
			fallbackModelId === primaryInfo.model.id
		) {
			return undefined;
		}
		...
		(fallbackConfig as Record<string, unknown>)[
			getProviderModelIdKey(fallbackProvider, "act")
		] = fallbackModelId;
	...
	private async activateToolUseFailureFallback(
		errorMessage: string,
	): Promise<boolean> {
		...
		this.taskState.toolUseFailureFallbackPrimaryProviderId =
			fallback.primaryProviderId;
		this.taskState.toolUseFailureFallbackPrimaryModelId =
			fallback.primaryModelId;
	...
	async recursivelyMakeClineRequests(
		...
		if (this.taskState.consecutiveMistakeCount >= maxConsecutiveMistakes) {
			...
			const shouldAutoRetryMistakeLimit =
				!this.taskState.suppressMistakeLimitAutoRetry &&
				this.taskState.mistakeLimitAutoRetryAttempts < 3;
			...
			if (shouldAutoRetryMistakeLimit) {
				...
				// Calculate delay: 2s, 4s, 8s
				const delay =
					2000 * 2 ** (this.taskState.mistakeLimitAutoRetryAttempts - 1);
				const errorMessage =
				...
				const didSuppressAutoRetryAfterFeedback =
					this.taskState.suppressMistakeLimitAutoRetry;
				const errorMessage = didSuppressAutoRetryAfterFeedback
					...
					: mistakeLimitMessage;
				const didActivateFallback =
					await this.activateToolUseFailureFallback(errorMessage);
				...
				if (didActivateFallback) {
					...
					const primaryModelId =
						this.taskState.toolUseFailureFallbackPrimaryModelId ?? model.id;
					const fallbackProviderId =
						this.taskState.toolUseFailureFallbackProviderId ??
						this.getCurrentProviderInfo().providerId;
					const fallbackModelId =
						this.taskState.toolUseFailureFallbackModelId ??
					...
					await this.say(
						...
						JSON.stringify({
							retrySource: "mistake_limit",
							retrySuppressed: didSuppressAutoRetryAfterFeedback
								? "after_user_feedback"
								: undefined,
							attempt: 3,
							maxAttempts: 3,
					...
					this.taskState.mistakeLimitAutoRetryAttempts = 0;
					this.taskState.suppressMistakeLimitAutoRetry =
						didReceiveMistakeLimitFeedback;
					this.taskState.autoRetryAttempts = 0;
					resetMistakeLimitState();
				}
			}
		...
		if (shouldCompact) {
			...
			this.taskState.activeContextManagementTool =
			...
			*/
			...
			if (assistantHasContent) {
				...
				if (!didToolUse) {
					...
					this.taskState.consecutiveMistakeCount = this.taskState
						.suppressMistakeLimitAutoRetry
						? maxConsecutiveMistakes
