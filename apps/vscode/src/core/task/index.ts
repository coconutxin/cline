...
export class Task {
	...
	private pauseTaskDurationTracking() {
		...
		this.accumulatedTaskDurationMs += Math.max(0, Date.now() - this.activeTaskDurationStartedAtMs)
	...
	private getTaskDurationMs(): number {
		...
		return this.accumulatedTaskDurationMs + Math.max(0, Date.now() - this.activeTaskDurationStartedAtMs)
	}
	...
	async ask(
		...
		if (didPauseDuration && !(type === "completion_result" && result.response === "yesButtonClicked")) {
	...
	private getCurrentProviderInfo(): ApiProviderInfo {
		...
		const providerId =
			activeProviderId === "cline-pass" && !featureFlagsService.getBooleanFlagEnabled(FeatureFlag.CLINE_PASS)
				? "cline"
				: activeProviderId
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		try {
			...
			if (isContextWindowExceededError && !this.taskState.didAutomaticallyRetryFailedApiRequest) {
				...
				const isEntitlementError = clineError.isErrorType(ClineErrorType.Entitlement)
				const isOrgClinePassRestrictionError = clineError.isErrorType(ClineErrorType.OrgClinePassRestriction)
				// ClinePass period limits reset in hours/days — auto-retrying is
				// pointless and only delays the actionable error UI.
				const isClinePassLimitError = clineError.isErrorType(ClineErrorType.ClinePassLimit)
				...
				const isClineFreeModelLimitError = clineError.isErrorType(ClineErrorType.ClineFreeModelLimit)
	...
	async recursivelyMakeClineRequests(userContent: ClineContent[], includeFileDetails = false): Promise<boolean> {
		...
		if (this.taskState.consecutiveMistakeCount >= maxConsecutiveMistakes) {
			...
			const mistakeLimitMessage = "Cline hit repeated tool call failures. Try guiding it with a new prompt."
		...
		const canAutoCondense = useAutoCondense && isNextGenModelFamily(this.api.getModel().id)
		...
		if (!shouldCompact && !this.taskState.activeContextManagementTool) {
			const getProjectedUsage = (content: ClineContent[]) => {
				...
				return this.contextManager.getProjectedContextWindowUsage(projectedConversationHistory, this.api)
			}
			...
			*/
			...
			try {
				...
				if (!this.taskState.abandoned) {
					...
					const isStreamingQuotaExceededError = clineError.isErrorType(ClineErrorType.QuotaExceeded)
					const isStreamingClineFreeModelLimitError = clineError.isErrorType(ClineErrorType.ClineFreeModelLimit)
					const isStreamingNonRetriableError =
						isStreamingSpendLimitError || isStreamingQuotaExceededError || isStreamingClineFreeModelLimitError
					// Auto-retry for streaming failures (skip for non-retriable errors)
					if (!isStreamingNonRetriableError && this.taskState.autoRetryAttempts < 3) {
						this.taskState.autoRetryAttempts++
					...
					} else if (!isStreamingNonRetriableError && this.taskState.autoRetryAttempts >= 3) {
