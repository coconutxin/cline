...
let taskHistoryRecoveryPromise: Promise<HistoryItem[]> | undefined

function parseTaskHistory(contents: string): HistoryItem[] {
	const parsed: unknown = JSON.parse(contents)
	if (!Array.isArray(parsed)) {
		throw new Error("Task history file must contain a JSON array")
	}

	return parsed as HistoryItem[]
}

function recoverTaskHistory(filePath: string, parseError: unknown): Promise<HistoryItem[]> {
	if (!taskHistoryRecoveryPromise) {
		telemetryService.captureExtensionStorageError(
			parseError instanceof Error ? parseError.message : String(parseError),
			"parseError_attemptingRecovery",
		)

		taskHistoryRecoveryPromise = (async () => {
			try {
				const result = await reconstructTaskHistory(false)
				if (result && result.reconstructedTasks > 0) {
					const reconstructedContents = await fs.readFile(filePath, "utf8")
					return parseTaskHistory(reconstructedContents)
				}
			} catch (error) {
				Logger.warn("[Disk] Failed to recover task history:", error)
			}

			// Ensure future startups read valid JSON even when there are no task folders to reconstruct.
			await writeTaskHistoryToState([])
			return []
		})().finally(() => {
			taskHistoryRecoveryPromise = undefined
		})
	}

	return taskHistoryRecoveryPromise
}
...
export async function readTaskHistoryFromState(): Promise<HistoryItem[]> {
	try {
		...
		try {
			return parseTaskHistory(contents)
		} catch (parseError) {
			return recoverTaskHistory(filePath, parseError)
		}
	} catch (error) {
