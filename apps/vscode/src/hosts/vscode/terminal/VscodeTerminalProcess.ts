		...
		if (terminal.shellIntegration && terminal.shellIntegration.executeCommand) {
			...
			const getCompletionFallbackSignal = () => {
				if (!completionFallbackSignal) {
					completionFallbackSignal = new Promise<"completionFallback">(
						(resolve) => {
							resolveCompletionFallbackSignal = () =>
								resolve("completionFallback");
						},
					);
				}
			...
			while (true) {
				...
				const currentCompletionFallbackSignal =
					completionFallbackSignal as Promise<"completionFallback"> | null;
				const next = currentCompletionFallbackSignal
					? await Promise.race([
							iterator.next(),
							currentCompletionFallbackSignal.then(() => fallbackResult),
						])
					: await iterator.next();
				...
				if ("completionFallback" in next) {
					...
					if (closeStream) {
						void Promise.resolve(closeStream).catch((error) => {
							Logger.error(
								"Error closing terminal shell integration stream after completion fallback:",
								error,
							);
						});
					}
