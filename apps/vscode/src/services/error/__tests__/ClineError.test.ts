...
describe("ClineError", () => {
	describe("getErrorType", () => {
		...
		it("should prefer ClinePassLimit over Auth for a 403 with the limit message", () => {
			...
			const err = new ClineError({
				message: "You have reached your 5-hour Clinepass limit. The limit resets in 5h, please try again later.",
				status: 403,
			})
		...
		it("should classify nested daily Cline free model limits as ClineFreeModelLimit", () => {
			...
			const err = new ClineError({
				...
				error: {
					message: "Daily free limit reached on model deepseek/deepseek-v4-flash. Try again in 5h 12m",
				},
			})
		...
		it("should prefer ClineFreeModelLimit over the generic rate-limit patterns", () => {
			...
			const err = new ClineError({
				message: "Error 429: Daily free limit reached on model deepseek/deepseek-v4-flash. Try again in 23h 59m",
				status: 429,
			})
	...
	describe("isClineFreeModelLimitMessage", () => {
		it("matches daily and generic free-limit messages", () => {
			...
			isClineFreeModelLimitMessage("Free limit reached on model cline-free/glm-5").should.be.true()
		})
	...
	describe("extractClineFreeModelLimitResetTime", () => {
		...
		it("returns undefined when there is no reset window", () => {
			;(
				extractClineFreeModelLimitResetTime("Daily free limit reached on model deepseek/deepseek-v4-flash.") === undefined
			).should.be.true()
		})
	})
	...
	describe("isClinePassLimitMessage", () => {
		...
		it("does not match unrelated or partial messages", () => {
			isClinePassLimitMessage("the user is not subscribed to required model plan").should.be.false()
			isClinePassLimitMessage(
	...
	describe("extractClinePassLimitMessage", () => {
		it("extracts the limit message out of a wrapped error string", () => {
			...
			extractClinePassLimitMessage(`429 Error: ${message}`)!.should.equal(message)
		})

		it("returns undefined when there is no limit message", () => {
			;(extractClinePassLimitMessage("some other error") === undefined).should.be.true()
		})
	})
})
