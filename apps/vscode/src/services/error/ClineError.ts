 ...
 */
export function extractClineFreeModelLimitResetTime(text: string): string | undefined {
	...
	const resetTime = message.slice(resetStart + CLINE_FREE_MODEL_LIMIT_RETRY_MARKER.length).trim()
...
export class ClineError extends Error {
	...
	static getErrorType(err: ClineError): ClineErrorType | undefined {
		...
		if (isClineFreeModelLimitMessage(detailMessage ?? "") || isClineFreeModelLimitMessage(message ?? "")) {
			return ClineErrorType.ClineFreeModelLimit
		}

		if (isClinePassLimitMessage(detailMessage ?? "") || isClinePassLimitMessage(message ?? "")) {
