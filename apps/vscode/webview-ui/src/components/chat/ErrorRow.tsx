...
import { ClineError, ClineErrorType, extractClinePassLimitMessage } from "../../../../src/services/error/ClineError"
		...
		switch (errorType) {
			...
			case "mistake_limit_reached":
				...
				if (rawApiError) {
					...
					if (clineError?.isErrorType(ClineErrorType.ClinePassLimit) && providerId === "cline-pass") {
						const detailMessage = clineError?._error?.details?.message || errorMessage
						const limitMessage = extractClinePassLimitMessage(detailMessage) ?? detailMessage
