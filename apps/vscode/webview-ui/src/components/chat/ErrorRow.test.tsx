...
describe("ErrorRow", () => {
	...
	describe("API error handling", () => {
		...
		it("renders ClinePass limit error and switches to Cline usage-based billing", async () => {
			...
			expect(request.apiConfiguration.planModeApiProvider).toBe(ApiProvider.CLINE)
			expect(request.apiConfiguration.actModeApiProvider).toBe(ApiProvider.CLINE)
		...
		it("does not offer the usage-based billing switch when already on the cline provider", async () => {
			...
			expect(screen.queryByTestId("cline-pass-limit-error")).not.toBeInTheDocument()
			expect(screen.getByText(limitMessage)).toBeInTheDocument()
		})
		...
		it("offers the paid twin of the selected free model", async () => {
			const limitMessage = "Daily free limit reached on model cline-free/deepseek-v4-flash. Try again in 23h 59m"
			const mockClineError = {
				message: limitMessage,
				isErrorType: vi.fn((type) => type === "clineFreeModelLimit"),
				providerId: "cline",
			...
			;(mockApiConfiguration as Record<string, unknown>).actModeClineModelId = "cline-free/deepseek-v4-flash"
			...
			try {
				...
				const request = mockUpdateApiConfigurationProto.mock.calls[0][0]
				expect(request.apiConfiguration.actModeApiProvider).toBe(ApiProvider.CLINE)
				expect(request.apiConfiguration.actModeClineModelId).toBe("deepseek/deepseek-v4-flash")
			} finally {
				mockApiConfiguration.actModeApiProvider = "cline-pass"
				delete (mockApiConfiguration as Record<string, unknown>).actModeClineModelId
			}
		})
