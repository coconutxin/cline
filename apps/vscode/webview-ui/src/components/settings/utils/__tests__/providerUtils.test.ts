...
describe("providerUtils", () => {
	...
	describe("syncModeConfigurations openai-codex", () => {
		it("copies the model and Fast service tier across plan and act modes", async () => {
			const apiConfiguration: ApiConfiguration = {
				planModeApiProvider: "openai-codex",
				actModeApiProvider: "openai-codex",
				actModeApiModelId: "gpt-5.6-sol",
				actModeOpenAiCodexServiceTier: "priority",
			}
			...
			expect(updates?.planModeApiModelId).toBe("gpt-5.6-sol")
			expect(updates?.actModeApiModelId).toBe("gpt-5.6-sol")
			expect(updates?.planModeOpenAiCodexServiceTier).toBe("priority")
			expect(updates?.actModeOpenAiCodexServiceTier).toBe("priority")
		})
	})
})
